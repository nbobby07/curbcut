...
test('B — scan/list/inspect exercises the real sandbox, store, source selection, and highlight', async ({ page }) => {
  ...
  const highlighted = page.frameLocator('iframe[title="Rendered source preview"]')
    .locator('[data-curbcut-highlight="true"]')
  await expect(highlighted).toHaveCount(1)
  await expect(highlighted).toHaveAttribute('id', 'email')
  await expect(page.getByTestId('activity-timeline')).toContainText('label · mapped line 16')
...
test('E — apply/rescan/summary/undo/rescan completes the exact shared workflow', async ({ page }) => {
  ...
  await expect(timeline).toContainText('6 rules · 6 nodes · 3 critical · 3 serious')
  await expect(timeline).toContainText('CONTEXTUAL · 1 edit · not applied · approval required')
  await expect(timeline).toContainText('source changed · revision 2 · rescan required')
  await expect(timeline).toContainText('1 applied · 1 verified · 0 undone')
  await expect(timeline).toContainText('source restored · revision 3 · rescan required')
})
