...
import type { IsolationEvidence } from './previewProtocol'
import {
...
export type VerificationNotice = {
  changeId: string
  kind: 'APPLY' | 'UNDO'
  outcome: 'PENDING' | 'VERIFIED' | 'NOT_VERIFIED' | 'RESTORED' | 'NOT_RESTORED'
  message: string
}
...
function edit(kind: 'html' | 'css', value: string) {
...
export const workspaceStore = {
  ...
  async scan(reason: 'initial' | 'after_change' | 'manual', signal?: AbortSignal): Promise<CommandResult<ScanRecord>> {
    if (cancelled(signal)) return fail('CANCELLED', 'The WebMCP execution was cancelled.')
    ...
    try {
      ...
      const scan = { scanId, sourceRevision: revision, metrics: getScanMetrics(issues), reason }
      ...
      if (latest?.undoneRevision === revision) {
        ...
        history[history.length - 1] = { ...latest, verification: remains ? 'NOT_VERIFIED' : 'VERIFIED' }
        verificationNotice = {
          changeId: latest.changeId,
          kind: 'APPLY',
          outcome: remains ? 'NOT_VERIFIED' : 'VERIFIED',
          message: remains
      ...
      update({ scanStatus: 'CURRENT', scan, issues, history, verificationNotice, selectedIssueId: null, highlightedNodeId: null })
  ...
  async previewRepair(issueId: string, family: RepairFamily, humanValues: HumanValues, signal?: AbortSignal) {
    ...
    update({ proposal: result.data, previewMode: 'PROPOSED', error: null, verificationNotice: null })
  ...
  async applyProposal(proposalId: string, signal?: AbortSignal) {
    ...
    const result = await applyApprovedProposal(proposal, {
  ...
  rejectProposal(proposalId?: string) {
    ...
    update({ proposal: result.data, previewMode: 'WORKING', error: null })
    return { ok: true as const, data: result.data }
  },
  async undoLatest(signal?: AbortSignal) {
    ...
    const result = await undoLatestChange(latest, { html: state.htmlSource, css: state.cssSource })
    if (cancelled(signal)) return fail('CANCELLED', 'The WebMCP execution was cancelled.')
    if (!result.ok) {
      update({ error: result.error.message })
  ...
  async exportSource(kind: ExportKind): Promise<CommandResult<ExportMetadata>> {
    ...
    try {
      ...
      return fail('EXPORT_FAILED', error instanceof Error ? error.message : 'The browser blocked the source download.')
    }
  },
